...
function showRanking() {
    $(".rankingPlayer1").html(
        "<h1 class='rankingh1'>" + myname + " : " + myScore + "</h1>",
    )
    $(".rankingPlayer2").html(
        "<h1 class='rankingh1'>" +
            oppname +
            " : " +
            (Totalgames - myScore - draw) +
            "</h1>",
    )
}
...
socket.on("wrongName", (data) => {
    console.log(data)
    alert(
        "We have detected that you have provided a different name from the previous Game. Avoid Doing that in further games to avoid confusion.",
    )
})
...
document.getElementById("leftWait").addEventListener("click", () => {
    $(".left").hide()
    $(".notice").html(
        "<h1>Waiting for Opponent to reconnect. Room id: " + room + "</h1>",
    )
    $(".notice").show()
    $(".ranking").show()
    $(".Ox").show()
...
socket.on("winner", (data) => {
    if (data.whoStarted == Player) {
        didIStartFirst = true
    } else {
        didIStartFirst = false
    }
    console.log("Who started:   " + data.whoStarted)
